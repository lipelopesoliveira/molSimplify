...
def get_primitive(datapath, writepath, occupancy_tolerance=1):
    ...
    # Output structure to a file.
    sprim.to(writepath, fmt="cif")
...
def load_sbu_lc_descriptors(sbupath):
    ...
    # Checking if there is a file there.
...
def gen_and_append_desc(temp_mol, target_list, depth, descriptor_names, descriptors, feature_type):
    ...
    results_dictionary = generate_atomonly_autocorrelations(temp_mol,
                                                            target_list,
                                                            depth=depth,
                                                            loud=False,
                                                            oct=False,
    ...
    descriptor_names, descriptors = append_descriptors(descriptor_names,
                                                       descriptors,
    ...
    results_dictionary = generate_atomonly_deltametrics(temp_mol,
                                                        target_list,
                                                        depth=depth,
                                                        loud=False,
                                                        oct=False,
...
def make_MOF_SBU_RACs(SBUlist,
    ...
    connections_list : list of lists of int
        Each inner list is its own separate linker. The ints are the atom indices of that linker.
        Length is # of linkers.
    ...
    for i, SBU in enumerate(SBUlist):
        ...
        """""""""
        # Iterating over the different linkers
        for j, linker in enumerate(connections_list):
            ...
            if len(set(SBU).intersection(linker)) > 0:
                ...
                # Will hold the lc atoms for the current linker.
                link_list = []

                # linker is a list of global atom indices for the atoms in that linker
                ...
                """""""""
                results_dictionary, descriptor_names, descriptors = gen_and_append_desc(temp_mol,
                                                                                        link_list,
                                                                                        depth,
                                                                                        descriptor_names,
                                                                                        descriptors,
                                                                                        'lc')
                """""""""
                If heteroatom functional groups exist (anything that is not C or H, so methyl is missed,
                also excludes anything lc, so carboxylic metal-coordinating oxygens skipped),
                compile the list of atoms
                """""""""

                functional_atoms = []
                for jj in range(len(temp_mol.graph)):
                    # linker atom is not bonded to a metal
                    if jj not in link_list:
                        # not a carbon nor a hydrogen. syms get symbols.
                ...
                if len(functional_atoms) > 0:
                    results_dictionary, descriptor_names, descriptors = gen_and_append_desc(temp_mol,
                                                                                            ...
                                                                                            descriptors,
                                                                                            'func')
                # There are no functional atoms.
                else:
                    descriptor_names, descriptors = append_descriptors(descriptor_names,
                                                                       descriptors,
                                                                       results_dictionary['colnames'],
                                                                       list([np.zeros(int(6*(depth + 1)))]),
                                                                       'func',
                                                                       'all')
                ...
                if lc_descriptors.shape[0] == 0:
                    ...
                    lc_descriptors = pd.concat([lc_descriptors, pd.DataFrame(desc_dict, index=[0])], ignore_index=True)
                lc_descriptor_list.append(descriptors)
                if j == 0:
                    lc_names = descriptor_names

        # Average the lc RACs over all of the lc atoms in the MOF.
        ...
        sbu_index_connection_indices = list(np.sort(sbu_index_connection_indices))
        ...
        descriptors.remove(name)
        descriptor_names.remove('name')
        if sbu_descriptors.shape[0] == 0:
            sbu_descriptors = pd.DataFrame(desc_dict, index=[0])
        else:
            sbu_descriptors = pd.concat([sbu_descriptors, pd.DataFrame(desc_dict, index=[0])], ignore_index=True)
    ...
    # Average the SBU RACs over all of the SBUs in the MOF.
...
def make_MOF_linker_RACs(linkerlist,
    ...
    for i, linker in enumerate(linkerlist):
        ...
        # Sort in ascending order
        linker_index_connection_indices = list(np.sort(linker_index_connection_indices))
        ...
        for ii, properties in enumerate(allowed_strings):
            ...
            # This is the case when just starting and the list is empty.
        ...
        if linker_descriptors.shape[0] == 0:
            linker_descriptors = pd.DataFrame(desc_dict, index=[0])
        else:
            linker_descriptors = pd.concat([linker_descriptors, pd.DataFrame(desc_dict, index=[0])], ignore_index=True)
...
def make_MOF_func_RACs(molcif,
    ...
    if descriptor_path:
        # Checking if there is a file there.
        if os.path.exists(descriptor_path + '/func_descriptors.csv'):
            func_descriptors = pd.read_csv(descriptor_path + '/func_descriptors.csv')
        else:
            func_descriptors = pd.DataFrame()
    ...
    for j, linker in enumerate(connections_list):
        ...
        for val in linker:
            temp_mol.addAtom(molcif.getAtom(val))
        ...
        if len(functional_atoms) > 0:
            ...
            # There are no functional atoms.
            print('No functional atoms found!')
        ...
        if func_descriptors.shape[0] == 0:
            func_descriptors = pd.DataFrame(desc_dict, index=[0])
        else:
            func_descriptors = pd.concat([func_descriptors, pd.DataFrame(desc_dict, index=[0])], ignore_index=True)

        func_descriptors.to_csv(descriptor_path + '/func_descriptors.csv', index=False)

    return list(desc_dict.keys()), list(desc_dict.values())
...
def bond_information_write(linker_list, linkeranchors_superlist, adj_matrix, molcif, cell_v, path):
    ...
    # Iterate over all linkers
    for linker_idx, linker_atoms_list in enumerate(linker_list):
        # Getting the connection points of the linker
        for anchor_super_idx in range(len(linkeranchors_superlist)):
            # Any anchor index in the current entry of linkeranchors_superlist is in the current linker's indices
            if list(linkeranchors_superlist[anchor_super_idx])[0] in linker_atoms_list:
                # Indices of the connection points in the linker
                linker_connection_points = list(linkeranchors_superlist[anchor_super_idx])
        for con_point in linker_connection_points:
            ...
            # The indices of atoms connected to atom with index con_point.
            connected_atoms = np.nonzero(connected_atoms)[0]
            ...
            for con_atom in connected_atoms:
                ...
                if con_atom3D.ismetal(transition_metals_only=False):
                    ...
                    # Shifting the connected metal
                    fcoords[con_atom] += compute_image_flag(cell_v, fcoords[con_point], fcoords[con_atom])
    ...
    mean_bond_len = np.mean(bond_length_list) 
...
def surrounded_sbu_gen(SBU_list, linker_list, sbupath, molcif, adj_matrix, cell_v, allatomtypes, name):
    ...
    for SBU_idx, atoms_sbu in enumerate(SBU_list):
        ...
        # List of the coordinating atoms of each of the connected linkers. Length is # of connected linkers.
        connection_atoms = []
        # List of the atoms of each of the connected linkers. Length is # of connected linkers
        atoms_connected_linkers = []
        ...
        # For each atom index in an inner list in connection_atoms,
        # build out the corresponding linker (inner list) in atoms_connected_linkers

        # Start with the atoms of the SBU
        surrounded_sbu = mol3D()  # SBU surrounded by linkers
        starting_atom_idx = atoms_sbu[0]
        # This list will contain the SBU indices that no longer need to be considered for branching.
...
def get_MOF_descriptors(data,
                        ...
                        wiggle_room=1, max_num_atoms=2000,
    ...
    path : str
        ...
        This output also includes the xyz and net files for the cif being analyzed,
        written with the function writeXYZandGraph.
    xyzpath : str
    ...
    # Throw an error if the user did not supply a path to which to write the output.
    if not path:
    ...
    # Don't deal with large cifs because of computational resources required for their treatment.
    if len(cart_coords) > max_num_atoms:
    ...
    """""""""
    # Make the adjacency matrix if not provided.
    ...
    # Grab the adjacency matrix from the cif file.
    else:
        adj_matrix_list = []
        max_sofar = 0
    ...
    # molcif is a mol3D class of a single unit cell (or the cell of the cif file)
    ...
    n_components, labels_components = sparse.csgraph.connected_components(csgraph=adj_matrix,
                                                                          directed=False,
                                                                          return_labels=True)
    ...
    # There are multiple connected components that have a metal in them in this case.
    if n_components > 1:
    ...
    # Consider all metals as part of the SBUs.
    [SBUlist.update(set([metal])) for metal in molcif.findMetal(transition_metals_only=False)]

    # atoms connected to metals. # TODO why use this over adj_matrix?
    [SBUlist.update(set(molcif.getBondedAtomsSmart(metal))) for metal in molcif.findMetal(transition_metals_only=False)]
    removelist = set()

    # Remove all metals as part of the SBUs.
    [removelist.update(set([metal])) for metal in molcif.findMetal(transition_metals_only=False)]
    for metal in removelist:
        bonded_atoms = set(molcif.getBondedAtomsSmart(metal))
        # The types of elements bonded to metals. E.g. oxygen, carbon, etc.
    ...
    [removelist.update(set([atom])) for atom in SBUlist if
     all((molcif.getAtom(val).ismetal() or molcif.getAtom(val).symbol().upper() == 'H')
         for val in molcif.getBondedAtomsSmart(atom))]

    """""""""
    adding hydrogens connected to atoms which are only connected to metals.
    In particular interstitial OH, like in UiO SBU.
    """""""""
    ...
    step 2: organic part
        removelist = linkers are all atoms - the removelist (assuming no bond between organic linkers)
    """""""""

    # The indices of all the atoms.
    ...
    # The number of atoms in each linker.
    linker_length_list = [len(linker_val) for linker_val in linker_list]

    """""""""
    find all anchoring atoms on linkers and ligands (lc identification) The atoms that are bonded to a metal.
    """""""""

    anc_atoms = set()
    # Checking all of the linkers one by one.
    for linker in linker_list:
        # Checking each atom in the current linker.
        for atom_linker in linker:
            # indices of atoms with bonds to the atom with the index atom_linker
            bonded2atom = np.nonzero(adj_matrix[atom_linker, :])[1]
            # This means one of the atoms bonded to the atom with the index atom_linker is a metal.
            if set(bonded2atom) & metal_list:
                anc_atoms.add(atom_linker)

    """""""""
    step 3: determine whether linker or ligand checking to find the anchors and #SBUs that are connected to an organic part
    anchor <= 1 -> ligand
    ...
    # Will contain the indices of the linker atoms that coordinate to metals
    linkeranchors_superlist = []
    # Loop over all linker subgraphs
    for ii, atoms_list in reversed(list(enumerate(linker_list))):
        linkeranchors_list = set()
        linkeranchors_atoms = set()
        sbuanchors_list = set()
        # Will contain the indices of SBU subgraphs that have a connection to the current linker described by atoms_list
        ...
        # loop over all atom indices in a linker
        for iii, atoms in enumerate(atoms_list):
            # indices of atoms with bonds to the atom with the index atoms
            connected_atoms = np.nonzero(adj_matrix[atoms, :])[1]
            # loop over all SBU subgraphs
            for kk, sbu_atoms_list in enumerate(initial_SBU_list):
                # Loop over SBU
                for sbu_atoms in sbu_atoms_list:
                    # found an SBU atom bonded to an atom in the linker defined by atoms_list
                    if sbu_atoms in connected_atoms:
                        linkeranchors_list.add(iii)
                        linkeranchors_atoms.add(atoms)
                        sbuanchors_list.add(sbu_atoms)
                        # Add if unique SBUs
                        sbu_connect_list.add(kk)
        ...
        # linker, and in one ambiguous case, could be a ligand.
        if len(linkeranchors_list) >= 2:
            # Something that connects two SBUs is certain to be a linker
            if len(sbu_connect_list) >= 2:
                ...
                # Cartesian coordinates of the atoms in the linker
                ...
                # Periodic images for the organic component
                ...
                # Adding atoms. Not sure why the [0] is there? TODO
                ...
                # Periodic images for the SBU component
                pr_image_sbu = ligand_detect(cell_v, sbu_cart_coords, sbu_adjmat, set(range(len(linkeranchors_list))))

                # linker. More than one periodic image for sbu or organic component
                if not (len(np.unique(pr_image_sbu, axis=0)) == 1 and len(np.unique(pr_image_organic, axis=0)) == 1):
                    max_min_linker_length = max(min_length, max_min_linker_length)
                    min_max_linker_length = min(max_length, min_max_linker_length)
                    tmpstr = f'{name}, Anchors list: {sbuanchors_list}, SBU connectlist: {sbu_connect_list} set to be linker\n'
                    write2file(ligandpath, "/ambiguous.txt", tmpstr)
                    continue
                #  all anchoring atoms are in the same unitcell -> ligand
                else:
                    # we also want to remove these ligands
                    removelist.update(set(templist[ii]))

                    # we also want to remove these SBUs
                    SBUlist.update(set(templist[ii]))
                    linker_list.pop(ii)
                    linker_subgraphlist.pop(ii)

                    tmpstr = f'{name}, Anchors list: {sbuanchors_list}, SBU connectlist: {sbu_connect_list} set to be ligand\n'
                    write2file(ligandpath, "/ambiguous.txt", tmpstr)
                    tmpstr = f'{name}{ii}, Anchors list: {sbuanchors_list}, SBU connectlist: sbu_connect_list\n'
        ...
        # Definite ligand
            ...
            linker_list.pop(ii)
            linker_subgraphlist.pop(ii)
            tmpstr = f'{name}, Anchors list: {sbuanchors_list}, SBU connectlist: {sbu_connect_list}\n'
            write2file(ligandpath, "/ligand.txt", tmpstr)
    ...
    """""""""
    In the case of long linkers, add second coordination shell without further checks.
    In the case of short linkers, start from metal and grow outwards using the include_extra_shells function
    """""""""
    ...
    # treating the 2 atom ligands differently! Need caution
    if not min_max_linker_length < 2:
        if long_ligands:
    ...
    # Featurize cases that did not fail.
    if len(linker_subgraphlist) >= 1:
        # try:
        descriptor_names, descriptors, lc_descriptor_names, lc_descriptors = make_MOF_SBU_RACs(SBU_list,
                                                                                               SBU_subgraphlist,
                                                                                               molcif,
                                                                                               depth,
                                                                                               name,
        ...
        lig_descriptor_names, lig_descriptors = make_MOF_linker_RACs(linker_list,
                                                                     linker_subgraphlist,
                                                                     molcif,
                                                                     depth,
                                                                     name,
    ...
    # Only one linker identified.
    elif len(linker_subgraphlist) == 1:
        print('Suspicious featurization')
        full_names = [1]
        full_descriptors = [1]

    # Means len(linker_subgraphlist) is zero.
    else:
...
def get_COF_descriptors(data,
    ...
    # The indices of all the atoms.
    linkers = set(range(0, adj_matrix.shape[0]))
    linker_list, linker_subgraphlist = get_closed_subgraph(linkers.copy(), set(), adj_matrix)
    ...
    if len(linker_subgraphlist) >= 1:
        ...
        func_descriptor_names, func_descriptors = make_MOF_func_RACs(molcif,
                                                                     depth,
                                                                     name,
                                                                     path,
                                                                     connections_list=linker_list,
                                                                     connections_subgraphlist=linker_subgraphlist)

        full_names = lig_descriptor_names + func_descriptor_names
        full_descriptors = list(lig_descriptors) + list(func_descriptors)
    ...
    # Means len(linker_subgraphlist) is zero.
        ...
        print('Failed to featurize this COF; no linkers were identified.')
        full_names = [0]
        full_descriptors = [0]
